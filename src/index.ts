...
const getBinList = async () => {
    try {
        const json: { optionalDependencies: object } = JSON.parse(
...
const checkBinList = (binList: object | null) => {
    if (binList === null) {
        return false;
    }
...
const getBinPath = async () => {
    try {
        const module: { default: string } = await import(currentBin.pkg);
...
const checkBinAccess = async (path: string | null) => {
    if (path === null) {
        return false;
    }
    try {
